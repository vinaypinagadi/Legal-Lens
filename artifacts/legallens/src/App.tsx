import { type ButtonHTMLAttributes, type ChangeEvent, type FormEvent, type ReactNode, useState } from 'react';
import { QueryClient, QueryClientProvider } from '@tanstack/react-query';
import {
  ArrowUpRight, BookOpen, Check, ChevronRight, CircleAlert, Clipboard, CloudOff,
  FileText, Library, LoaderCircle, Menu, MessageSquareText, Plus,
      ...
      <aside className={cx('fixed inset-y-8 left-0 z-40 w-[252px] border-r border-border bg-card/90 px-5 py-6 backdrop-blur-xl transition-transform lg:sticky lg:top-8 lg:block lg:h-[calc(100dvh-2rem)] lg:translate-x-0', mobileOpen ? 'translate-x-0' : '-translate-x-full')}>
        ...
        <div className="mt-12">
          ...
          <nav className="space-y-1">
            ...
            <NavItem href="/settings" icon={<Settings2 size={17} />} label="AI & privacy" onClick={() => setMobileOpen(false)} />
          </nav>
        </div>
        <div className="absolute bottom-6 left-5 right-5 rounded-xl border border-primary/15 bg-primary/[.06] p-4">
          ...
          <p className="mt-1 text-[11px] leading-relaxed text-muted-foreground">Documents stay in your workspace. AI calls use the secure server connection.</p>
...
function Workspace() {
  ...
  const runAnalysis = () => { if (!document) return; analyze.mutate({ documentId, data: { model } }, { onSuccess: () => { queryClient.invalidateQueries({ queryKey: getGetDocumentQueryKey(documentId) }); queryClient.invalidateQueries({ queryKey: getListDocumentsQueryKey() }); queryClient.invalidateQueries({ queryKey: getGetOverviewQueryKey() }); } }); };
  const askQuestion = (event: FormEvent) => { event.preventDefault(); const q = question.trim(); if (!q) return; setQuestion(''); ask.mutate({ documentId, data: { question: q, model } }, { onSuccess: () => queryClient.invalidateQueries({ queryKey: getListChatMessagesQueryKey(documentId) }) }); };
  ...
  return <Shell><div className="mx-auto max-w-[1240px]">
    ...
    {document.status !== 'ready' && <div className="mt-6 flex flex-col gap-4 rounded-xl border border-primary/20 bg-primary/[.045] p-5 sm:flex-row sm:items-center sm:justify-between"><div><div className="flex items-center gap-2 font-semibold"><Sparkles size={16} className="text-primary" /> Ready for a careful first pass?</div><p className="mt-1 text-sm text-muted-foreground">LegalLens uses its secure server-side Gemini connection only when you choose to analyze.</p></div><Button data-testid="button-analyze-document" onClick={runAnalysis} disabled={analyze.isPending}>{analyze.isPending ? <><LoaderCircle size={16} className="animate-spin" /> Reading…</> : <><Sparkles size={16} /> Analyze contract</>}</Button></div>}
    <div className="mt-4 flex items-center gap-3 rounded-lg border border-primary/20 bg-primary/[.045] px-4 py-3 text-sm"><ShieldCheck size={16} className="shrink-0 text-primary" /><span><strong>Secure AI connection.</strong> Your contract is sent to Gemini only when you start analysis or ask a question.</span><Link href="/settings" data-testid="link-ai-settings" className="ml-auto shrink-0 font-semibold text-primary hover:underline">Privacy <ArrowUpRight size={13} className="inline" /></Link></div>
    {analyze.isError && <p data-testid="status-analysis-error" className="mt-4 rounded-lg bg-destructive/10 px-4 py-3 text-sm text-destructive">Analysis could not be completed. Please try again.</p>}
    <div className="mt-7">
      ...
      {activeTab === 'summary' && <div className="grid gap-7 xl:grid-cols-[minmax(0,1.35fr)_minmax(330px,.65fr)]"><section className="min-w-0"><SummaryCard document={document} /><details className="mt-6 rounded-xl border border-border bg-card p-5"><summary className="cursor-pointer list-none font-semibold">View source text</summary><SourceText text={document.sourceText} /></details></section><aside className="min-w-0"><ChatPanel messages={messages} question={question} setQuestion={setQuestion} suggestions={promptSuggestions} onAsk={askQuestion} loading={ask.isPending} disabled={document.status !== 'ready'} /></aside></div>}
      {activeTab === 'risks' && <RiskReview risks={document.risks} status={document.status} />}
      {activeTab === 'chat' && <ChatPanel messages={messages} question={question} setQuestion={setQuestion} suggestions={promptSuggestions} onAsk={askQuestion} loading={ask.isPending} disabled={document.status !== 'ready'} />}
    </div>
  </div></Shell>;
}
